...
Sites-liteでは実際の抽選後に「双星の誓約」の正位置、「銀河の環冠」と「軌道を持たない彗星」の逆位置の画像・日本語本文を確認した。知識セットの既存カードも表示できた。Geminiの外部API呼び出しは今回の検証対象に含めていない。GitHub反映は次の手順で実施する。
