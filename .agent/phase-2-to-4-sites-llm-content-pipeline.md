...
- [x] (2026-07-26) HTTP APIでカードID・contentVersionをサーバー側コンテンツへ正規化し、出力トークン上限、タイムアウト、再試行、429/API停止フォールバックを検証した。
- [x] (2026-07-26) 品質CLIへ禁止テーマ、高リスク断定、正逆差分、助言長、キーワード類似、画像形式・寸法・SHA-256、公開レビュー承認ゲートを追加した。
- [x] (2026-07-26) ブラウザソースへGemini秘密値が混入しない静的検査を追加した。
